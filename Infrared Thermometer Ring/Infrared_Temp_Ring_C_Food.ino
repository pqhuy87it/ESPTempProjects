...
void loop() {
  float foodTTemperature = mlx.readObjectTempC();
  display.clearDisplay();
  if (foodTTemperature < 50) // Put the minimum temp
{    display.setTextSize(1);
    display.setTextColor(SSD1306_WHITE);
    display.setCursor(6, 25);
    display.print("LOW SERVE TEMP<50C");
  } else if (foodTTemperature >= 50 && foodTTemperature <= 60) //Put the Good and to hot temp
{
    display.setTextSize(1);
    display.setTextColor(SSD1306_WHITE);
    display.setCursor(6, 25);
    display.print("GOOD SERVE TEMP>50C");
  } else
{
    display.setTextSize(1);
    display.setTextColor(SSD1306_WHITE);
    display.setCursor(6, 25);
    display.print("HIGH SERVE TEMP>60");
  }
